...
def create_renko_router(db, definedge, get_current_subscriber) -> APIRouter:
    ...
    async def _chart_commodity_intraday(symbol: str, interval: str, box_pct: Optional[float],
                                         box_value: Optional[float], cfg, ma_period: int, days: int) -> dict:
        """Gold, intraday only — see pnf_routes.py's identical function and
        mt5_client.py's module docstring for the full reasoning."""
        sym = symbol.strip().upper()
        if sym != mt5c.SYMBOL:
            raise HTTPException(status_code=404, detail=f"No instrument found for {symbol}.")
    ...
    async def chart(symbol: str, segment: str = "NSE", interval: str = "daily",
        ...
        if segment == "COMMODITY":
            ...
            return await _chart_commodity_intraday(symbol, interval, box_pct, box_value, cfg, ma_period, days)
        if segment == "CRYPTO":
            raise HTTPException(status_code=400,
