...
// Loads the Razorpay Checkout script once and caches the promise -- it's a
// widget bundle, not an SDK, and Razorpay's own docs load it this way (no
// npm package for the client-side checkout modal itself).
...
export default function PnfStudio() {
  ...
  const pay = async () => {
    ...
    try {
      ...
      const rzp = new window.Razorpay({
        ...
        description: `P&F Studio — ${cycle.label}`,
        prefill: { email: user?.email },
        theme: { color: "#1F5FD0" },
        handler: async (response) => {
          try {
            await axios.post(`${API}/pnf-access/verify`, response, authHeaders());
            toast.success("Payment received — P&F Studio is unlocked.");
            await refreshMe();
          } catch (err) {
  ...
  return (
    <>
      ...
      <main className="relative bg-void min-h-screen">
        <section className="relative pt-36 pb-16 md:pt-44 md:pb-20 overflow-hidden" data-testid="pnf-studio-hero">
        ...
        <section className="relative pb-28 md:pb-40">
          <div className="container-x grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-10 lg:gap-16 items-start max-w-5xl mx-auto">
            ...
            <Reveal delay={0.1} className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 md:p-8" data-testid="pnf-studio-access-panel">
              ...
              {status === "logged-out" && (
                <div data-testid="pnf-studio-logged-out">
              ...
              {status === "no-access" && (
                <div data-testid="pnf-studio-no-access">
                  ...
                  <div className="mt-6 inline-flex rounded-full border border-white/10 p-1" data-testid="pnf-studio-cycle-toggle">
                    {CYCLES.map((c) => (
                      <button
                        key={c.key}
                        ...
                        data-testid={`pnf-studio-cycle-${c.key}`}
                      >
                        {c.label}
                      </button>
                    ))}
                  ...
                  <button
                    ...
                    data-testid="pnf-studio-subscribe-btn"
                  >
                    {paying ? <><Loader2 size={16} className="animate-spin" /> Processing</> : `Subscribe — $${cycle.price}`}
                  </button>
                  <p className="mt-3 text-xs text-slate-500 leading-relaxed">
                    Secure checkout via Razorpay. Access activates immediately after payment.
                  </p>
                </div>
              )}

              {status === "active" && (
                <div data-testid="pnf-studio-active">
                  ...
                  <button
                    onClick={() => navigate("/alpha-terminal/pnf")}
                    className="btn-sapphire w-full mt-6 inline-flex items-center justify-center gap-2"
                    data-testid="pnf-studio-open-btn"
                  >
                    Open P&amp;F Studio <ArrowUpRight size={15} />
                  </button>
                </div>
              )}
            </Reveal>
