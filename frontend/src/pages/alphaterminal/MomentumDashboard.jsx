...
const SymbolMultiSelect = ({ universe, selected, onChange, max = 10 }) => {
  ...
  return (
    <div>
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3" data-testid="momentum-selected-chips">
          {selected.map((sym) => (
            <span key={sym} className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-slate-300 font-mono-ui">
              {sym}
              <button type="button" onClick={() => removeSymbol(sym)} className="text-slate-500 hover:text-red-400" data-testid={`momentum-remove-${sym}`}>×</button>
            </span>
          ))}
        </div>
      )}
      <div className="relative">
        <input
          ...
          disabled={atMax}
          data-testid="momentum-symbol-search"
        />
          ...
          <div className="absolute z-20 mt-1 w-full max-h-64 overflow-y-auto rounded-md border border-white/10 bg-[#0A0D18] shadow-xl">
            {matches.map((s) => (
              <button
                ...
                data-testid={`momentum-option-${s.symbol}`}
              >
...
const SORT_ACCESSORS = {
  ...
  momentum_score: (r) => (r.stats.momentum_score ?? -Infinity),
  return_12_1: (r) => (r.stats.return_12_1 ?? -Infinity),
  volatility: (r) => (r.stats.volatility ?? -Infinity),
};
...
const ResultsTable = ({ results }) => {
  ...
  return (
    <div className="glass rounded-2xl overflow-hidden" data-testid="momentum-results-table">
      <div className="hidden md:block">
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-white/10">
              <Header k="symbol">Symbol</Header>
              <Header k="momentum_score">Momentum Score</Header>
              <Header k="return_12_1">12-1 Return</Header>
              <Header k="volatility">Volatility</Header>
            </tr>
          </thead>
          <tbody>
            {sorted.map((r, i) => (
          ...
          <div key={r.symbol} className="p-5" data-testid={`momentum-card-${i}`}>
            ...
            <div className="flex items-center gap-6 text-sm">
              <div>
                ...
                <span className="text-slate-300">{fmtNum(r.stats.momentum_score)}</span>
              </div>
              <div>
                <p className="font-mono-ui text-[10px] uppercase tracking-[0.18em] text-slate-500 mb-1">12-1 Return</p>
                <span className={(r.stats.return_12_1 ?? 0) >= 0 ? "text-emerald-400" : "text-red-400"}>{fmtPctStat(r.stats.return_12_1)}</span>
              </div>
              <div>
                <p className="font-mono-ui text-[10px] uppercase tracking-[0.18em] text-slate-500 mb-1">Volatility</p>
                <span className="text-slate-300">{fmtPctStat(r.stats.volatility)}</span>
              </div>
            </div>
          </div>
        ))}
...
const MomentumDashboardTool = () => {
  ...
  return (
    <div className="mt-6" data-testid="momentum-tool">
      <div className="flex gap-2 mb-6">
        {MODES.map((m) => (
          <button
            ...
            data-testid={`momentum-mode-${m.key}`}
          >
            {m.title}
          </button>
        ))}
        ...
        {mode === "compare" ? (
          <>
            ...
            <button
              ...
              data-testid="momentum-compare-submit"
            >
          ...
          <div className="flex items-end gap-4">
            ...
            <button
              ...
              data-testid="momentum-top-submit"
            >
      ...
      {loading && <LoadingParticles title="Computing Momentum" subtitle="Fetching history · Scoring 12-1 momentum · Ranking" />}

      {!loading && result && !result.found && (
        <div>
          <EmptyState reason={result.reason || "No result found."} />
          {mode === "top" && refreshStatus?.status === "running" && (
            <p className="text-center text-xs text-slate-500 mt-3 font-mono-ui" data-testid="momentum-refresh-progress">
      ...
      {!loading && result && result.found && (
        <div data-testid="momentum-results">
          ...
          {mode === "compare" && result.skipped?.length > 0 && (
            <p className="text-xs text-amber-400/80 mb-4 font-mono-ui" data-testid="momentum-skipped">
              Skipped: {result.skipped.map((s) => `${s.symbol} (${s.reason})`).join(", ")}
            </p>
          )}
          <ResultsTable results={result.results} />
          <p className="text-[11px] font-light text-slate-600 mt-4 max-w-2xl">
            Momentum Score is the trailing 12-month return (excluding the most recent month) divided by annualized volatility over the same window — a risk-adjusted read that favors steady trends over choppy ones. Past performance doesn't guarantee future results — not investment advice.
          </p>
        </div>
      )}
    </div>
...
export default MomentumDashboardTool;
